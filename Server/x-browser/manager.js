...
class HostedBrowserManager {
  ...
  async _startInner({ headed = false } = {}) {
    ...
    const { requireResidentialPlaywrightProxy } = require('../lib/social/proxy');
    ...
    console.log(
      `[x-browser] Started (${this.headed ? 'headed' : 'headless'}) profile=${userDataDir} proxy=${this._proxyMeta.label || this._proxyMeta.server}`,
    );
    return this.status();
  }
  ...
  async _closeContext() {
    ...
    try {
      ...
      console.warn('[x-browser] Context close warning:', err?.message || err);
    }
  }
  ...
  async logout() {
    if (this.context) {
      try {
        ...
        console.warn('[x-browser] clearCookies:', err?.message || err);
      }
    }
    await this._closeContext();
    ...
    console.log('[x-browser] Logged out — profile wiped');
    return this.status();
  }
  ...
  async postXThread(args = {}) {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          '[x-browser] Compose-thread failed, reply-chain fallback:',
          err.message || err,
        );
      }
