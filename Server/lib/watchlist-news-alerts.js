...
async function loadNewsRow(newsId) {
...
async function watchersForNews(news) {
  const r = await db.query(
      ...
      WHERE u.email_verified = TRUE
        ...
        AND NOT EXISTS (
          SELECT 1 FROM watchlist_news_email_sent s
           WHERE s.user_id = u.id AND s.news_id = $3
        )`,
    [news.company_id, news.news_ticker || news.ticker, news.id],
  );
  return r.rows;
}
...
async function sendNewsAlertToUser(news, user) {
  ...
  await db.query(
    `INSERT INTO watchlist_news_email_sent (user_id, news_id) VALUES ($1, $2)
     ON CONFLICT (user_id, news_id) DO NOTHING`,
    [user.user_id, news.id],
  );
}
 ...
 */
async function processWatchlistNewsEmail(newsId) {
  if (!process.env.RESEND_API_KEY) return { sent: 0, skipped: true };

  const news = await loadNewsRow(newsId);
  if (!news) return { sent: 0, reason: 'not_found' };

  const watchers = await watchersForNews(news);
  if (!watchers.length) return { sent: 0, reason: 'no_watchers' };

  let sent = 0;
  for (const user of watchers) {
    try {
      await sendNewsAlertToUser(news, user);
      sent++;
    } catch (err) {
      console.error(`[watchlist-news] Email failed ${user.email} news=${newsId}:`, err?.message || err);
    }
    if (SEND_DELAY_MS > 0) await sleep(SEND_DELAY_MS);
  }

  if (sent > 0) {
    console.log(`[watchlist-news] Sent ${sent} alert(s) for news #${newsId}`);
  }
  return { sent };
}

/** Queue processing to batch rapid enrichments. */
function queueWatchlistNewsEmail(newsId) {
  ...
  flushTimer = setTimeout(async () => {
    ...
    for (const id of ids) {
      try {
        await processWatchlistNewsEmail(id);
      } catch (err) {
        console.error(`[watchlist-news] process failed #${id}:`, err?.message || err);
      }
    }
  }, 2000);
}
...
async function processPendingWatchlistNewsEmails() {
  ...
  for (const row of r.rows) {
    const result = await processWatchlistNewsEmail(row.id);
    total += result.sent || 0;
  }
  return { sent: total };
}
