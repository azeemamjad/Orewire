...
const src = fs.readFileSync(path.join(PUBLIC_ROOT, 'index.html'), 'utf8');
...
for (const tab of tabs) {
  ...
  const out = pageShell(tab, body, tab === 'companies' || tab === 'dashboard' || tab === 'filings'
    ...
    : '');
  fs.writeFileSync(path.join(ADMIN_DIR, `${tab}.html`), out);
...
`;
fs.writeFileSync(path.join(ADMIN_DIR, 'js', 'shared.js'), sharedParts + '\n// Page scripts loaded per-page\n');

// Redirect index
fs.writeFileSync(path.join(PUBLIC_ROOT, 'index.html'), `<!DOCTYPE html>
