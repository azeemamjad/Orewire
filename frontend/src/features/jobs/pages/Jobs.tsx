        ...
        <div className="container-page section-y">
          <div className="flex items-end justify-between flex-wrap gap-4 mb-4">
            ...
            <div className="flex items-center gap-2">
              {isAuthenticated && (
                <Link to="/jobs/dashboard" className="inline-flex items-center justify-center gap-2 whitespace-nowrap text-sm font-medium px-4 py-2 rounded-md border border-border bg-surface hover:bg-muted h-11">
                  <FileText className="w-4 h-4 mr-1" /> My listings
                </Link>
              )}
              <button
                onClick={() => setShowPostForm(true)}
                className="inline-flex items-center justify-center gap-2 whitespace-nowrap text-sm font-medium px-4 py-2 rounded-md bg-accent text-accent-foreground hover:bg-accent/90 h-11"
              >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="flex w-full border px-3 py-2 ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 md:text-sm pl-10 h-12 bg-card rounded-md border-foreground/20 focus-visible:ring-accent"
            />
          </div>
        ...
        <div className="container-page section-y grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-8 space-y-4">
            ...
            {[...promoted, ...regular].map((job) => (
              <article key={job.id} className={`border bg-surface p-4 lg:p-5 hover:bg-background/60 transition-colors relative ${job.promoted ? "border-accent" : "border-border"}`}>
                {job.promoted && (
                  <div className="absolute -top-2.5 left-4 font-mono text-[10px] uppercase tracking-widest bg-accent text-accent-foreground px-2 py-0.5 font-bold inline-flex items-center gap-1">
                ...
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <span className={`font-mono text-[10px] uppercase tracking-widest px-1.5 py-0.5 font-bold ${typeStyle[job.jobType] || "bg-foreground text-background"}`}>{job.jobType}</span>
                      {job.discipline && <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground border border-border px-1.5 py-0.5">{job.discipline}</span>}
                      {job.tags.map((tag) => (
                        <span key={tag} className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{tag}</span>
                      ))}
                    </div>
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 whitespace-nowrap text-sm font-medium h-10 px-4 py-2 rounded-md bg-foreground text-background hover:bg-foreground/90 shrink-0"
                  >
          ...
          <aside className="lg:col-span-4 space-y-4">
            ...
            <div className="border border-accent bg-surface p-5">
              <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-2">Listing</div>
    ...
    <div className="max-w-[1200px] mx-auto px-4 lg:px-6 py-6 lg:py-8">
      ...
      {apps.length === 0 ? (
        ...
        <div className="border border-border bg-surface">
          {/* Header */}
          <div className="grid grid-cols-[1fr_150px_120px_100px] gap-3 px-5 py-2.5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground bg-muted/30 border-b border-border">
            <div>Position</div>
            <div>Company</div>
            <div>Applied</div>
            <div>Status</div>
          ...
          <div className="divide-y divide-border">
            {apps.map((app) => (
              <div key={app.applicationId} className="grid grid-cols-[1fr_150px_120px_100px] gap-3 px-5 py-3.5 items-center hover:bg-background/60 transition-colors">
                ...
                <span className={`font-mono text-[10px] uppercase tracking-widest px-1.5 py-0.5 font-bold border text-center ${statusColor[app.status] || "bg-muted text-foreground border-border"}`}>
...
const PostJobDialog = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 backdrop-blur-sm" onClick={onClose}>
      <div className="fixed left-[50%] top-[50%] z-50 grid w-full translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg max-w-2xl rounded-md" onClick={(e) => e.stopPropagation()}>
        ...
        {showAuth ? (
          <div>
            ...
            <form onSubmit={handleAuth} className="space-y-3">
              <input type="email" value={authEmail} onChange={(e) => setAuthEmail(e.target.value)} placeholder="Email" required className="flex h-10 w-full border border-input bg-background px-3 py-2 text-sm rounded-md placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2" />
              ...
              <button type="submit" disabled={authLoading} className="w-full h-10 bg-accent text-accent-foreground text-sm font-semibold hover:bg-accent/90 rounded-md disabled:opacity-50">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2">
              <button type="submit" disabled={posting} className="inline-flex items-center justify-center gap-2 whitespace-nowrap text-sm font-medium h-10 px-4 py-2 rounded-md bg-accent text-accent-foreground hover:bg-accent/90 disabled:opacity-50">
...
const ApplyDialog = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 backdrop-blur-sm" onClick={onClose}>
      <div className="fixed left-[50%] top-[50%] z-50 grid w-full translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg max-w-2xl rounded-md max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex flex-col space-y-1.5 text-center sm:text-left">
        ...
        {success ? (
          <div className="py-8 text-center space-y-3">
            ...
            <button onClick={onApplied} className="inline-flex items-center justify-center gap-2 text-sm font-medium h-10 px-6 py-2 rounded-md bg-accent text-accent-foreground hover:bg-accent/90 mt-2">
              Done
            </button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              {isAuthenticated ? (
                <div className="flex h-10 w-full border border-input bg-muted/30 px-3 py-2 text-sm rounded-md items-center text-muted-foreground">
                  {user?.email}
                </div>
              ) : (
            ...
            <div className="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2">
              <button type="button" onClick={onClose} className="inline-flex items-center justify-center text-sm font-medium h-10 px-4 py-2 rounded-md border border-border hover:bg-muted">Cancel</button>
              <button type="submit" disabled={submitting || !name} className="inline-flex items-center justify-center gap-2 text-sm font-medium h-10 px-4 py-2 rounded-md bg-accent text-accent-foreground hover:bg-accent/90 disabled:opacity-50">
